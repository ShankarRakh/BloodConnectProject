...
export default function LiveDonorMapSection({ initialDonorLocation, recipientLocation }) {
  ...
  // Simulate donor movement every 5 seconds
  useEffect(() => {
    if (isClient && initialDonorLocation && recipientLocation) {
  ...
  }, [isClient, initialDonorLocation, recipientLocation, stepIncrement, simulationInterval]);
  ...
  return (
<div className="h-[50vh] w-full relative -z-30">
{isClient && (
        <>
          <MapContainer center={center} zoom={13} style={{ height: "100%", width: "100%" }} ref={mapRef} className="z-5">
            <TileLayer
